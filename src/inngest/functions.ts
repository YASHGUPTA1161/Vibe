...
export const codeAgentFunction = inngest.createFunction(
  ...
  async ({ event, step}) => {
    const sandboxId = await step.run("get-sandbox-id", async () => {
      const sandbox = await Sandbox.create("vibe-nextjs-yash");
    ...
    //   description: "An expert title generator agent",
    ...
    //   if (fragmentTitleOupput[0].type !== "text") {
    //     return "Fragment";
    //   }
    //   const content = fragmentTitleOupput[0].content;
    //   if (Array.isArray(content)) {
    //     return content.map((txt) => txt).join("");
    //   } else {
    //     return content;
    //   }
    // };
